...
def hologramPos(currenthologramPos):
    ...
    #print("\nGoal_x: ", hologram.position.x, " Goal_y: ", hologram.position.y, " Goal_z: ", hologram.position.z)
...
def hologramTrack():
	...
	if(not rospy.is_shutdown() and 'stop' not in msg):
		...
		if((not waitNext) and holo_counter_x[len(holo_counter_x)-2] == holo_counter_x[len(holo_counter_x)-1] and ((abs(goalPose.position.x) > 0.2) or (abs(goalPose.position.y) > 0.2))):
			counter = timer()
			i+=1
		...
		if(waitNext and (abs(goalPose.position.x) <= 0.10) and (abs(goalPose.position.y) <= 0.10)):
...
def main():
	...
	global hologram, tello, goalPose, takeOff, pub_vel, drone_vel, zero_vel, msg, telloStatus, ID1, ID10, pub_tellopose, speed, flag, integral_x, integral_y, integral_z, holo_counter_x, holo_counter_y, holo_counter_z, i, holoCount, counter, waitNext, dataFile, y_integral_sum, saveTello, pub_status_color, current_status_color
	
	dataFile = open('src/thesis_project/data/datafile.txt','a')
	dataFile.write('\n')
	...
	sub_tello = rospy.Subscriber("/drone_pose", CurrentDronePose, telloPos)
